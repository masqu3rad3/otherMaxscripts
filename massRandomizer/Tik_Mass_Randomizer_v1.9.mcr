...
fn rndUVW Uoffmin Uoffmax Voffmin Voffmax Rotoffmin Rotoffmax = --function for random UV offsets
	undo on
for i in selection do 
(
...
rollout randomUV "UV offset Randomizer"
	(
        ...
        if selection.count > 0 then
        (
            rndUVW u1.value u2.value v1.value v2.value r1.value r2.value
        )
        else (messagebox "select at least one object")
        )
)
